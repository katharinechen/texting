...
- [ ] carybara test for users 
- [ ] user factorygirlrails  
- [ ] fix validations for Message 
- [ ] don't give errors on the log in page (when there is an error)
...
- [ ] Create a route and controller for creating inbound messages. For this feature, you should make a model that doesn't inherit from Active Record, since it doesn't need to save anything to the database. This type of model is often called a Plain Old Ruby Object, or PORO. Your PORO should take a phone number and text the automated response back to it. When the inbound text is received, it will take the request, pass the phone number of the sender to the model, and the model then sends the automated response back.
...
- [ ] As a user who sends a text message from the web, I want texts sent to my Twilio number in an online inbox, so I can see what people want to tell me. Note: this replaces the previous feature of auto-replying to texts with a canned response.

- [ ] Fix design for flash notifications 